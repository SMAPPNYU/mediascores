...
To enable within-chain parallelization^[Between-chain parallelization is enabled by default.] requires that the C++ code compiled during the [`mediascores`](https://github.com/SMAPPNYU/mediascores){target="_blank"} library's installation be done using flags to turn it on. The process to do this unfortunately varies from system to system (e.g. Windows, Unix, Mac), but is not overly complicated. For example, enabling parallelization on the systems to which the author has access required only that the following be added to the `~/.R/Makevars` file (create this file if it does not exist):
