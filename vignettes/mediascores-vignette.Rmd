...
<h2 style = "font-size: 26px; margin-top: 15px">The [`mediascores`](https://cran.r-project.org/package=mediascores) library in R</h2>
...
<h5 style = "font-size: 18px; margin-top: 30px; font-weight: 300">**[Author 1](https://google.com)**^$\dagger$^, **[Author 2](https://google.com)**^$\dagger$^, **[Author 3](https://google.com)**^$\dagger$^

^$\dagger$^ NYU [Social Media and Political Participation (SMaPP) Lab](https://smappnyu.org/) & Department of Politics</h5>
...
The object returned from the `mediascores()` function is a standard [Stan](https://mc-stan.org) object. For in depth analysis of the posterior, one can therefore treat it as such and use open-source visualization tools to examine it, such as [`shinystan`](https://mc-stan.org/users/interfaces/shinystan):
...
For more detail on $\hat{R}$, see the [Stan Modeling Language User Guide and Reference](https://mc-stan.org/users/documentation/).
...
To pull out these estimates, we can use the `point_est()` function, which calucates the median of the posterior of each parameter of interest, and provides credible intervals:
...
ggplot(M, aes(x = zeta_hat, y = media_org)) +
  ...
  geom_label(data = subset(M, label == "right"),
             ...
             label.padding = unit(0.07, "lines"), 
             hjust = 0, size = 2.5, nudge_x = 0.13) +
    theme(plot.margin = unit(c(0.5, 0.25, 0.5, -1), "lines"))
```
