...
<h5 style = "font-size: 18px; margin-top: 30px; font-weight: 300">**[Gregory Eady](https://google.com){target="_blank"}**^$\dagger$^ and **[Frido Linder](https://google.com){target="_blank"}**^$\dagger$^
...
# and column 2 Fox News. As a result lower values of theta_i and zeta_m will
# represent the liberal side of the scale; higher values, the conservative side.
\system.time{fitted_model <- mediascores(Y = User_Domain_Counts, group = Data$group,
                            variational = FALSE, user_variance = FALSE,
                            chains = 4, cores = 4, threads = 4,
                            warmup = 750, iter = 1500, seed = 1,
                            open_progress = TRUE,
                            anchors = c(1, 2))}
# saveRDS(fitted_model, "data/fitted_models/115Congress_Fitted_Model.rds")
fitted_model <- readRDS("data/fitted_models/115Congress_Fitted_Model.rds")
```
...
Here, we can see that none have values greater than 1.1. If we did, however, it is generally suggested that the researcher re-run the model, but collect more post-warmup iterations:
...
```{r}
Data$theta_hat <- theta_hat[, "median"]

# All politicians
round(cor(Data$theta_hat, Data$nominate, use = "complete.obs"), 2)

# Democrats
round(cor(Data$theta_hat[Data$party == "Democrat"], Data$nominate[Data$party == "Democrat"], use = "complete.obs"), 2)

# Republicans
round(cor(Data$theta_hat[Data$party == "Republican"], Data$nominate[Data$party == "Republican"], use = "complete.obs"), 2)
```

We can further graph these estimates as follows:

```{r echo = FALSE}
Data$role <- factor(Data$role, levels = c("Senate", "House", "Governor", "Politician-Other", "Other", "Delegate"))
```

```{r, eval = FALSE}
ggplot(subset(Data, role %in% c("Senate", "House")),
...
```{r, echo = FALSE, out.width = "100%", fig.height = 3.75, warning = FALSE}
ggplot(subset(Data, role %in% c("Senate", "House")),
       aes(x = theta_hat, y = nominate, color = party,
           fill = party, shape = party)) +
...
M <- data.frame(media_org = colnames(Data)[7:(ncol(Data)-1)],
