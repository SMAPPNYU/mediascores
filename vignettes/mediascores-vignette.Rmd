...
The [`mediascores`](https://cran.r-project.org/package=mediascores){target="_blank"} library is coded using map-reduce to permit within-chain CPU parallelization. This allows researchers to use many CPU cores for model fitting and thereby _substantially_ cut down on estimation time. This can be useful even for small-sized data sets, because the model is computationally demanding. For example, using a 4-core (7 year old) laptop, the model estimated in this vignette (using data from a 590 x 152 matrix) took roughly three and a half hours to fit. On a 20-core computer with within-chain parallelization turned on, model fitting took less than an hour.

To enable within-chain parallelization^[Between-chain parallelization is enabled by default.] requires that the C++ code compiled during the [`mediascores`](https://cran.r-project.org/package=mediascores){target="_blank"} library's installation be done using flags to turn it on. The process to do this unfortunately varies from system to system (e.g. Windows, Unix, Mac), but is not overly complicated. For example, enabling parallelization on the systems to which the author has access required only that the following be added to the `~/.R/Makevars` file (create this file if it does not exist):
