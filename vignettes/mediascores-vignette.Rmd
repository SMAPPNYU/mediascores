...
Below, we work through an application of the method to Twitter data there were collected in 2018 from Members of Congress during the 115^th^ session. Our example demonstrates how researchers can use such data to estimate the news-sharing ideology of social media users---in this case, politicians---and that of the content that they share online.
...
To begin, we read in the `data.frame` that contains the count of the stories from each news media domain that have been tweeted and retweeted by each politician. These data, which are included in the [`mediascores`](https://cran.r-project.org/package=mediascores){target="_blank"} library, also contain basic identifying information about each actor:
...
The resulting user-domain count matrix can then be inputted as an argument into the workhorse function of the [`mediascores`](https://cran.r-project.org/package=mediascores){target="_blank"} library, `mediascores()`, as follows:
...
Here, we focus on a simple helper function built into the [`mediascores`](https://cran.r-project.org/package=mediascores){target="_blank"} library to access one of the key post-estimation statistics, $\hat{R}$, which is commonly used to assess parameter convergence. Below, we use the `rhat()` function to display the $\hat{R}$ statistic for the two primary quantities of interest, $\vartheta_i$ (the news-sharing ideology of users), and $\zeta_m$ (the ideology of news media organizations):
...
Here, we see that none of the specified parameters have $\hat{R}$ statistics greater than 1.1. If we did, it is generally suggested that the model be re-run to collect more post-warmup iterations:
...
If after model fitting you receive a message that there were "divergent transitions" during estimation, this suggests potential validity concerns about the resulting parameter estimates. Details of this issue lie outside the purview of this vignette, but this problem can often be resolved by increasing the value of `adapt_delta`, which can be included in the call to the `mediascores()` function as follows: `mediascores(..., control = list(adapt_delta = 0.99))`. The default value is 0.8 (max < 1). Values of 0.99 or 0.999 typically resolve the problem. However, note that increasing the value of `adapt_delta` can substantially increase the time for model fitting, which can be long as is, especially with relatively large datasets. In practice, for the model presented in this library, this problem in our experience is rare.


## 5.2 Maximum treedepth exceeded {#subsection52}

If after model fitting you receive a message stating that the "maximum treedepth" was exceeded during estimation, the concern is not validity, but efficiency. If desired, you can increase efficiency by increasing the value of the `max_treedepth` argument, which defaults to 10. As with `adapt_delta`, this argument can be included in the function call as follows: `mediascores(..., control = list(max_treedepth = 15))`.
...
The [`mediascores`](https://cran.r-project.org/package=mediascores){target="_blank"} library is coded using map-reduce to permit within-chain CPU parallelization. The upshot is that you can use many CPU cores for model fitting, and thus _substantially_ cut down on estimation time. This is especially useful even for small-sized data sets, because the model is computationally demanding. For example, using a 4-core (7 year old) laptop, the model estimated above (using data in a 590 x 152 matrix), took roughly three and a half hours to fit. On a 20-core computer with parallelization turn on, model fitting took less than an hour.

To enable within-chain parallelization^[Between-chain parallelization is enabled by default.] requires that the C++ code compiled during the [`mediascores`](https://cran.r-project.org/package=mediascores){target="_blank"} library's installation be done using flags to turn it on. The process to do this varies from system to system (e.g. Windows, Unix, Mac), but is not overly difficult.

For example, enabling parallelization on the systems to which the author has access required only that the following be added to the `~/.R/Makevars` file (create this file if it does not exist):
