...
```{r}
load("~/GitHub/mediascores/data/115Congress.RData") # Make this local data
```
...
print(Data[Data$name %in% politicians_to_show, columns_to_show], row.names = FALSE)
```
...
User_Domain_Counts <- as.matrix(Data[, -c(1:6)])

# For reference, name rows by politician's name
rownames(User_Domain_Counts) <- Data$name
...
fitted_model <- mediascores(Y = User_Domain_Counts, group = Data$group,
...
Here, we can see that the parameters for which we have received warnings are just on the edge of the 1.1 threshold. In this case, we might therefore use the estimates as is or, to ensure that all parameters are below the 1.1 threshold, re-run the model using more iterations:
