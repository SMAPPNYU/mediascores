...
<h2 style = "font-size: 26px; margin-top: 15px">The [`mediascores`](https://cran.r-project.org/package=mediascores){target="_blank"} library in R</h2>
...
<h5 style = "font-size: 18px; margin-top: 30px; font-weight: 300">**[Author 1](https://google.com){target="_blank"}**^$\dagger$^, **[Author 2](https://google.com){target="_blank"}**^$\dagger$^, **[Author 3](https://google.com){target="_blank"}**^$\dagger$^
...
```{r eval = FALSE}
n_cores <- 4 # Set the number of physical cores on my system (will differ per user)
...
fitted_model <- mediascores(Y = User_Domain_Counts, group = D$group,
                            variational = FALSE, seed = 1,
                            warmup = 750, iter = 1500, chains = 1,
                            open_progress = TRUE,
...
Here, we focus on a simple helper function in the [`mediascores`](https://cran.r-project.org/package=mediascores){target="_blank"} library to access the convergence statistic, $\hat{R}$, which is the commonly used means of assessing whether one's parameters of interest have converged. Here we use the `rhat()` function to display the $\hat{R}$ statistic for the two typical quantities of interest, $\vartheta_i$ (the sharing ideology of users), and $\zeta_m$ (the ideology of news media domains):
...
The function outputs the $\hat{R}$ statistics for all the specified parameters, and includes a warning for any of those parameters that are greater than 1.1, the rule-of-thumb cutoff suggested by @Gelman2014. As we can see, for the fitted model, there are 10 values of $\hat{R}$ for the parameters $\zeta_m$ that are greater than 1.1.

We can look at the parameters $\zeta_m$ with values greater than 1.1 as follows:

```{r}
subset(rhats, rhat > 1.1)
```

Here, we can see that the parameters for which we have received warnings are just on the edge of the 1.1 threshold. In such cases, one can use estimates as is, or to ensure that all parameters are below the 1.1 threshold, re-run the model to collect more iterations:
...
```{r}
D$theta_hat <- theta_hat[, "median"]

# All politicians
cor(D$theta_hat, D$nominate, use = "complete.obs")

# Democrats
cor(D$theta_hat[D$party == "Democrat"], D$nominate[D$party == "Democrat"], use = "complete.obs")

# Republicans
cor(D$theta_hat[D$party == "Republican"], D$nominate[D$party == "Republican"], use = "complete.obs")
```

We can further graph these estimates as follows:

```{r echo = FALSE}
D$role <- factor(D$role, levels = c("Senate", "House", "Governor", "Politician-Other", "Other", "Delegate"))
```

```{r, eval = FALSE}
ggplot(subset(D, role %in% c("Senate", "House")),
...
M <- data.frame(media_org = colnames(D)[7:(ncol(D)-1)],
...
```{r, echo = FALSE, fig.height = 15, out.width = "100%", warning = FALSE}
M$label <- "right"
M$label[M$zeta_hat > median(M$zeta_hat)] <- "left"
...
On the NYU High-Performance Computing cluster (CentOS 7), the following was added to the `~/.R/Makevars` file:
